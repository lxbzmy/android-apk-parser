...
/**
 * 这个类为了提高效率采用了类将字段依序展开成数组的办法。
 * 每一组有6个, 所以data 的长度是len×6
...
//    /*package*/ int[] mData;
     ...
     * @param index
     */
    void addValueAt(TypedValue value,int index){
        values[index] = value;
        indexCount++;
    }
    ...
    public CharSequence getText(int index) {
        ...
        final int type = this.values[index].type;
        ...
        TypedValue v = this.values[index];
        if (v!=null) {
...
//              + Integer.toHexString(type));
    ...
    public String getString(int index) {
        final TypedValue v = this.values[index];
...
//            }
    ...
    public String getNonConfigurationString(int index, int allowedChangingConfigs) {
        TypedValue item = this.values[index];
        final int type = item.type;
        if ((item.changingConfigurations&~allowedChangingConfigs) != 0) {
            return null;
        }
        if (type == TypedValue.TYPE_NULL) {
            return null;
        ...
        if (item.type != TypedValue.TYPE_NULL) {
            Log.w(Resources.TAG, "Converting to string: " + item);
            CharSequence cs = item.coerceToString();
            return cs != null ? cs.toString() : null;
        }
        Log.w(Resources.TAG, "getString of bad type: 0x"
              + Integer.toHexString(type));
...
//              + Integer.toHexString(type));
    ...
    public float getFloat(int index, float defValue) {
        ...
        if (type == TypedValue.TYPE_NULL) {
            ...
            return Float.intBitsToFloat(this.values[index].data);
        } else if (type >= TypedValue.TYPE_FIRST_INT
            && type <= TypedValue.TYPE_LAST_INT) {
...
//    }
    ...
    public float getDimension(int index, float defValue) {
        ...
        if (type == TypedValue.TYPE_NULL) {
            ...
            return TypedValue.complexToDimension(
                    this.values[index].data, mResources.mMetrics);
        }
    ...
    public float getFraction(int index, int base, int pbase, float defValue) {
        ...
        if (type == TypedValue.TYPE_NULL) {
            ...
            return TypedValue.complexToFraction(
                    this.values[index].data, base, pbase);
        }
    ...
    public int getResourceId(int index, int defValue) {
        final TypedValue value = this.values[index];
        if(value.type!=TypedValue.TYPE_NULL){
            final int resid = value.resourceId;
            if (resid != 0) {
                return resid;
            }
        }
...
//        return getValueAt(index*AssetManager.STYLE_NUM_ENTRIES, outValue);
        TypedValue inner = this.values[index];
        outValue.assetCookie = inner.assetCookie;
        outValue.changingConfigurations = inner.changingConfigurations;
        outValue.data = inner.data;
        outValue.density = inner.density;
        outValue.resourceId = inner.resourceId;
        outValue.string = inner.string;
        outValue.type = inner.type;
        return true;
    }
    ...
    public TypedValue peekValue(int index) {
        TypedValue typedValue = this.values[index];
...
//    }
    ...
    private CharSequence loadStringValueAt(int index) {
        final int cookie = this.values[index].assetCookie;
        if (cookie < 0) {
            if (mXml != null) {
                return mXml.getPooledString(
...
//            cookie, data[index+AssetManager.STYLE_DATA]);
    ...
    /*package*/ TypedArray(Resources resources, int len) {
        mResources = resources;
        mLength = len;
        this.values = new TypedValue[len];
    }
